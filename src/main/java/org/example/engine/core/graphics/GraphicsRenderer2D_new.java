...
public class GraphicsRenderer2D_new implements MemoryResourceHolder {
    ...
    public void begin(GraphicsCamera camera) {
        if (drawing) throw new IllegalStateException("Already in a drawing state; Must call " + GraphicsRenderer2D_new.class.getSimpleName() + ".end() before calling begin().");
    ...
    public void end() {
        if (!drawing) throw new IllegalStateException("Called " + GraphicsRenderer2D_new.class.getSimpleName() + ".end() without calling " + GraphicsRenderer2D_new.class.getSimpleName() + ".begin() first.");
        flush();
    ...
    private static GraphicsShaderProgram createDefaultShaderProgram() {
        try (InputStream vertexShaderInputStream = GraphicsRenderer2D_new.class.getClassLoader().getResourceAsStream("graphics-2d-default-shader.vert");
             ...
             InputStream fragmentShaderInputStream = GraphicsRenderer2D_new.class.getClassLoader().getResourceAsStream("graphics-2d-default-shader.frag");
