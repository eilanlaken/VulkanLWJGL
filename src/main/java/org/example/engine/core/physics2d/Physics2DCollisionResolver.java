...
public class Physics2DCollisionResolver {
    ...
    protected void resolve(Physics2DWorld.CollisionManifold manifold) {
        ...
        for (int i = 0; i < manifold.contacts; i++) {
            ...
            float raCrossN = MathVector2.crs(ra_list.get(i), manifold.normal);
            float rbCrossN = MathVector2.crs(rb_list.get(i), manifold.normal);
            float d = invMassSum + (raCrossN * raCrossN) * aInertiaInv + (rbCrossN * rbCrossN) * bInertiaInv;
