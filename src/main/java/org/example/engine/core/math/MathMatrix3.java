...
public class MathMatrix3 {
    ...
    public MathMatrix3(MathMatrix3 matrix) {
        set(matrix);
        tmp[M22] = 1;
    }
     ...
     *           is not modified.) */
    public MathMatrix3(float[] values) {
        this.set(values);
        tmp[M22] = 1;
    }
    ...
    public MathMatrix3 mul (MathMatrix3 m) {
        float[] val = this.val;
     ...
     * @return This matrix for the purpose of chaining operations. */
    public MathMatrix3 setToRotation (float degrees) {
     ...
     * @return This matrix for the purpose of chaining operations. */
    public MathMatrix3 setToRotationRad (float radians) {
    ...
    public MathMatrix3 setToRotation(MathVector3 axis, float degrees) {
        return setToRotation(axis, MathUtils.cosDeg(degrees), MathUtils.sinDeg(degrees));
    }

    public MathMatrix3 setToRotation(MathVector3 axis, float cos, float sin) {
        float[] val = this.val;
        float oc = 1.0f - cos;
     ...
     * @return This matrix for the purpose of chaining operations. */
    public MathMatrix3 setToTranslation(float x, float y) {
        float[] val = this.val;
     ...
     * @return This matrix for the purpose of chaining operations. */
    public MathMatrix3 setToTranslation(MathVector2 translation) {
        float[] val = this.val;
     ...
     * @return This matrix for the purpose of chaining operations. */
    public MathMatrix3 setToScaling(float scaleX, float scaleY) {
        float[] val = this.val;
        val[M00] = scaleX;
        val[M10] = 0;
        val[M20] = 0;
     ...
     * @return This matrix for the purpose of chaining operations. */
    public MathMatrix3 setToScaling(MathVector2 scale) {
        float[] val = this.val;
        val[M00] = scale.x;
        val[M10] = 0;
        val[M20] = 0;
     ...
     * @return This matrix for the purpose of chaining operations.
     * */
    public MathMatrix3 inv() {
        float det = det();
        if (det == 0) throw new RuntimeException(MathMatrix3.class.getSimpleName() + " " + this + System.lineSeparator() + " not invertible. (det == 0)");
    ...
    public MathMatrix3 set(MathMatrix3 mat) {
     ...
     * @return This matrix for the purpose of chaining operations. */
    public MathMatrix3 set(MathMatrix4 mat) {
        float[] val = this.val;
        val[M00] = mat.val[MathMatrix4.M00];
        val[M10] = mat.val[MathMatrix4.M10];
        val[M20] = mat.val[MathMatrix4.M20];
        val[M01] = mat.val[MathMatrix4.M01];
        val[M11] = mat.val[MathMatrix4.M11];
        val[M21] = mat.val[MathMatrix4.M21];
        val[M02] = mat.val[MathMatrix4.M02];
        val[M12] = mat.val[MathMatrix4.M12];
        val[M22] = mat.val[MathMatrix4.M22];
        return this;
    }
    ...
    public MathMatrix3 set(float[] values) {
    ...
    public MathMatrix3 trn(MathVector2 vector) {
        val[M02] += vector.x;
        val[M12] += vector.y;
        return this;
    }
     ...
     * @return This matrix for the purpose of chaining. */
    public MathMatrix3 trn(float x, float y) {
        val[M02] += x;
        val[M12] += y;
        return this;
    }
     ...
     * @return This matrix for the purpose of chaining. */
    public MathMatrix3 trn(MathVector3 vector) {
        val[M02] += vector.x;
        val[M12] += vector.y;
        return this;
    }
     ...
     * @param y The y-component of the translation vector.
     * @return This matrix for the purpose of chaining. */
    public MathMatrix3 translate(float x, float y) {
        float[] tmp = this.tmp;
        tmp[M00] = 1;
        tmp[M10] = 0;
        // tmp[M20] = 0;
     ...
     * @param translation The translation vector.
     * @return This matrix for the purpose of chaining. */
    public MathMatrix3 translate(MathVector2 translation) {
        float[] tmp = this.tmp;
        tmp[M00] = 1;
        tmp[M10] = 0;
        // tmp[M20] = 0;
     ...
     * @param degrees The angle in degrees
     * @return This matrix for the purpose of chaining. */
    public MathMatrix3 rotate(float degrees) {
     ...
     * @param radians The angle in radians
     * @return This matrix for the purpose of chaining. */
    public MathMatrix3 rotateRad(float radians) {
     ...
     * @return This matrix for the purpose of chaining. */
    public MathMatrix3 scale(float scaleX, float scaleY) {
        float[] tmp = this.tmp;
        tmp[M00] = scaleX;
        tmp[M10] = 0;
        // tmp[M20] = 0;
     ...
     * @return This matrix for the purpose of chaining. */
    public MathMatrix3 scale(MathVector2 scale) {
        float[] tmp = this.tmp;
        tmp[M00] = scale.x;
        tmp[M10] = 0;
        // tmp[M20] = 0;
    ...
    public MathVector2 getTranslation(MathVector2 position) {
        position.x = val[M02];
        position.y = val[M12];
        return position;
    }
    ...
    public MathVector2 getScale(MathVector2 scale) {
        float[] val = this.val;
     ...
     * @return This matrix for the purpose of chaining methods together. */
    public MathMatrix3 scl(float scale) {
        val[M00] *= scale;
        val[M11] *= scale;
        return this;
    }
     ...
     * @param scale The {@link MathVector3} to use to scale this matrix.
     * @return This matrix for the purpose of chaining methods together. */
    public MathMatrix3 scl(MathVector2 scale) {
        val[M00] *= scale.x;
        val[M11] *= scale.y;
        return this;
    }
    ...
    /** Scale this matrix using the x and y components of the vector but leave the rest of the matrix alone.
     * @param scale The {@link MathVector3} to use to scale this matrix. The z component will be ignored.
     * @return This matrix for the purpose of chaining methods together. */
    public MathMatrix3 scl(MathVector3 scale) {
        val[M00] *= scale.x;
        val[M11] *= scale.y;
        return this;
    }
