...
public class CollectionsQueue<T> implements Iterable<T> {
    ...
    public CollectionsQueue(int initialSize) {
    ...
    public CollectionsQueue(int initialSize, Class<T> type) {
        // noinspection unchecked
    ...
    public boolean equals (Object o) {
        ...
        if (o == null || !(o instanceof CollectionsQueue)) return false;

        CollectionsQueue<?> q = (CollectionsQueue<?>)o;
        final int size = this.size;
    ...
    static public class QueueIterator<T> implements Iterator<T>, Iterable<T> {
        private final CollectionsQueue<T> queue;
...
// QueueIterable<T> iterable;
        ...
        public QueueIterator (CollectionsQueue<T> queue) {
            this(queue, true);
        }

        public QueueIterator (CollectionsQueue<T> queue, boolean allowRemove) {
            this.queue = queue;
...
// java.io.StringWriter lastAcquire = new java.io.StringWriter();
        ...
        public QueueIterable (CollectionsQueue<T> queue) {
            this(queue, true);
        }

        public QueueIterable (CollectionsQueue<T> queue, boolean allowRemove) {
            this.queue = queue;
            this.allowRemove = allowRemove;
        }
